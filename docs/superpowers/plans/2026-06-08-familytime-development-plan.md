...
## Stage 13.5 — сводка внимания (2026-09-11)

- [x] 13.5.1: общий приоритет дел, событий и поручений, создание и редактирование.
- [x] 13.5.2: отдельная загрузка относительно настоящего времени.
- [x] 13.5.3: подтверждения, просроченное, важное в ближайшие 7 суток; фильтрация закрытого.
- [x] 13.5.4: отдельный блок «Завтра» без дублирования важных записей.
- [x] 13.5.5: сортировка по группе, приоритету и сроку; первые 5 и раскрытие.

Проверка браузером: Today с январём 2025 показывает актуальную сводку;
повышение приоритета через модалку переносит запись из «Завтра» в важное
через realtime без смены открытого месяца. Проверены 1440x900 и 390x844.

## Stage 13.6 — списки, серии и PWA (2026-09-11)

- [x] 13.6.2: фильтр приоритета в делах, поручениях и серверном поиске.
- [x] 13.6.3: редактирование будущего расписания серии, транзакция и сохранение исключений.
- [x] 13.6.4: восстановление после потери сети и подтверждаемое обновление PWA.

Серверный regression: scripts/smoke-backend-isolated.mjs smoke-series.mjs
smoke-recurrence.mjs smoke-backend-security.mjs. Browser smoke использует
отдельный origin для двух версий service worker и удаляет свои тестовые записи.
