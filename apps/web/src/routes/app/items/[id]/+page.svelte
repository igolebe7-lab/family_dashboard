<script lang="ts">
  ...
  $: loadItem($page.params.id, context?.familyId, context?.memberId);
...
<WorkspacePage title={item?.title ?? 'Запись'}>
  <a class="back-link" href="/app/search"><ArrowLeft size={18} aria-hidden="true" />Все записи</a>
  {#if loading}<p class="page-empty-state" role="status">Загружаем запись…</p>{/if}
  {#if error}<div class="page-error" role="alert"><p>{error}</p>{#if !item}<button class="button button--soft" on:click={() => loadItem($page.params.id)}>Повторить</button>{/if}</div>{/if}
  ...
  {#if item}
    ...
    {:else}
      ...
      {#if item.kind === 'event' && context}<EventSchedule {item} {context} canEdit={Boolean(canEdit) && !item.archived && !archiving} />{/if}
