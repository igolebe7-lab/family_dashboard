...
В MVP можно реализовать годовой масштаб как заглушку/поздний milestone, но архитектуру календаря не ломать: события должны быть запрошены по диапазону дат.
...
- Календарь день/неделя — вертикальная временная шкала.
- Месяц — компактная сетка с точками.
...
- `year` heatmap;
- pinch-to-zoom между масштабами.

### 20.2. Mobile week/day
...
- controls: today, prev, next, date range;
- view switch: day/week/month;
- search;
- `+ Создать`;
- week grid with columns;
- right sidebar.
