...
Редактирование серии событий меняет только будущее расписание: время,
периодичность, интервал и окончание. Сохраняются уже материализованные
прошедшие экземпляры, индивидуальные переносы, отмены и даты с комментариями.
Обычные будущие экземпляры заменяются атомарно; новое начало должно быть
в будущем. Защищённые исходные дни записываются в recurrence_exdates_json,
чтобы не создавать рядом с переносом дубликат по новому правилу.
Endpoint PATCH /api/familytime/items/{id}/series проверяет права взрослого,
ожидаемые прежние параметры расписания и ограничивает обработку 5000
экземплярами; материализация нового правила ограничена 90 днями.
