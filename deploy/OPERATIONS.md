...
`previous` обновляет приёмник после успеха.

Начиная со Stage 14.12.4, каждый релиз содержит собственные immutable assets и
assets двух предыдущих отличающихся сборок. Root-owned приёмник ведёт
`asset-generations.json` вне загружаемого пакета и не переносит транзитивно всю
историю. Одинаковые наборы assets не вытесняют предыдущие версии. Совпадение имени
при различном содержимом отклоняется. Старый релиз без manifest временно считается
одним legacy-поколением; оно исчезнет после двух следующих отличающихся сборок.
Открытым вкладкам старше этого окна может потребоваться обновление; offline PWA
сохраняет уже закешированные assets. Текущий и rollback-релизы не модифицируются.
...
Stage 14.12.2: в ручном запуске есть `notification_test`. Он создаёт отдельную
issue `[TEST 14.12.2]`, назначенную владельцу, не имитируя падение production.
Создание issue не доказывает доставку email/баннера: владелец должен подтвердить
получение и затем закрыть тестовую issue. Реальные incidents обрабатываются отдельно.
Независимый сервис Stage 14.12.3 отложен по решению владельца 2026-09-16.

Для дополнительной копии ключа выбран Apple Passwords, запись
`FamilyTime — ключ восстановления backup`, пользователь `age-recovery`, без общего
доступа. Сам ключ хранится в поле пароля, а не в заметках. Синхронизация iCloud
зависит от настроек владельца; проверить наличие записи на iPhone. Не удалять
исходный identity.age и не публиковать его содержимое.
